...
def move_in(
    ...
    """
    First-time move-in. Backend calls after Solana multisig confirms
    NFT ownership and initial shards are assigned.
    """
    self._not_paused()
    ...
    log VaultMoveIn(
        ...
        occupant=msg.sender,
        signing_wallet=_signing_wallet,
...
def transfer_lease(
    ...
    """
    Transfers lease to new owner. Called by backend after:
    1. Solana confirms new owner holds the NFT
    2. Old shards invalidated
    3. New shards generated
    4. All 5 multisig members sign off
    Same slot, same contents — only owner and signing wallet change.
    """
    self._only_protocol()
    self._valid_slot(_slot_index)
    ...
    log LeaseTransferred(
        slot_index=_slot_index,
        old_owner=old_owner,
        new_owner=_new_owner,
        new_signer=_new_signer
    )

@external
def open_session(_slot_index: uint256, _duration_seconds: uint256):
    """Backend opens session after Solana multisig consensus."""
    self._only_protocol()
    self._valid_slot(_slot_index)
    self._slot_occupied(_slot_index)
...
def close_session(_slot_index: uint256):
    """Owner or backend closes the session."""
    self._valid_slot(_slot_index)
    self._slot_occupied(_slot_index)
...
def set_read_only(_slot_index: uint256, _reason: String[64]):
    """Protocol sets read-only when SELF_MODE owner ignores locker alert."""
    self._only_protocol()
    self._valid_slot(_slot_index)
    self._slot_occupied(_slot_index)
    ...
    log VaultReadOnly(slot_index=_slot_index, reason=_reason)

@external
def lift_read_only(_slot_index: uint256):
    """
    Lifts read-only. Protocol calls after Solana confirms
    NFT presence + holding wallet signature.
    """
    self._only_protocol()
    self._valid_slot(_slot_index)
    self._slot_occupied(_slot_index)
    ...
    log VaultReadOnlyLifted(slot_index=_slot_index)

@external
def set_security_mode(_slot_index: uint256, _mode: uint8):
    """Owner changes security mode. Requires active session."""
    self._valid_slot(_slot_index)
    self._slot_occupied(_slot_index)
    ...
    log SecurityModeChanged(slot_index=_slot_index, new_mode=_mode)
...
def get_slot(_slot_index: uint256) -> (bool, address, address, uint8, bool, bool):
    """Returns occupied, occupant, signing_wallet, security_mode, is_read_only, session_active"""
    self._valid_slot(_slot_index)
    return (
