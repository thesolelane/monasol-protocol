...
//   • pledge            — operator signals guardian phase has started;
//                         drives Active → Pledged
//   • confirm_settlement — oracle (backend) confirms Monad settlement tx;
//                          drives Pledged → Settling → Released
//                          CPIs: guardian_multisig::mark_settled
//                                vault_key::thaw_key (Settling)
//   • finalize_release  — called after Monad settlement confirmed on-chain;
//                         drives Released state cleanup
//                         CPIs: vault_key::move_out (burn NFT)
//                               guardian_multisig::close_guardian_set
//   • emergency_halt    — operator override for disputed/failed sessions;
//                         halts any non-Closed session
//
...
/// vault_key program ID — used for CPI discriminator construction.
pub const VAULT_KEY_PROGRAM_ID: &str =
    "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS";

/// guardian_multisig program ID.
...
/// These are the known discriminators for vault_key and guardian_multisig.
/// If instruction names change, these must be recomputed.
///
/// Discriminator computation (reference):
///   sha256("global:thaw_key")[..8]   = [see below]
///   sha256("global:move_out")[..8]   = [see below]
///   sha256("global:mark_settled")[..8] = [see below]
///   sha256("global:close_guardian_set")[..8] = [see below]
///
/// NOTE: Run `anchor idl parse` on each program to get exact discriminators
/// after first build on the server.  Placeholders here are computed from
/// the instruction names — replace with output of:
...
fn build_vault_key_thaw_ix(
    ...
    // discriminator for vault_key::thaw_key
    // recompute after first build: sha256("global:thaw_key")[..8]
    ...
    let accounts = vec![
        ...
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(*protocol_auth, true),
...
fn build_vault_key_move_out_ix(
    ...
    // discriminator for vault_key::move_out
    // recompute after first build: sha256("global:move_out")[..8]
    ...
    let accounts = vec![
        anchor_lang::solana_program::instruction::AccountMeta::new(*payer, true),
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(*protocol_auth, true),
        anchor_lang::solana_program::instruction::AccountMeta::new(*vault_session, false),
        anchor_lang::solana_program::instruction::AccountMeta::new(*asset, false),
        anchor_lang::solana_program::instruction::AccountMeta::new(*collection, false),
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(
            mpl_core::ID, false,
...
fn build_guardian_mark_settled_ix(
    ...
    // discriminator for guardian_multisig::mark_settled
    // recompute after first build: sha256("global:mark_settled")[..8]
    let discriminator: [u8; 8] = [0x3f, 0x4e, 0x5d, 0x6c, 0x7b, 0x8a, 0x91, 0xa0];

    let data = discriminator.to_vec();

    let accounts = vec![
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(*caller, true),
        anchor_lang::solana_program::instruction::AccountMeta::new(*guardian_set, false),
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(
            *system_program, false,
        ),
    ];
...
fn build_guardian_close_set_ix(
    ...
    // discriminator for guardian_multisig::close_guardian_set
    // recompute after first build: sha256("global:close_guardian_set")[..8]
    let discriminator: [u8; 8] = [0xb1, 0xc2, 0xd3, 0xe4, 0xf5, 0x06, 0x17, 0x28];

    let data = discriminator.to_vec();

    let accounts = vec![
        anchor_lang::solana_program::instruction::AccountMeta::new(*fee_payer, true),
        anchor_lang::solana_program::instruction::AccountMeta::new(*guardian_set, false),
        anchor_lang::solana_program::instruction::AccountMeta::new_readonly(
            *system_program, false,
        ),
