...
type PageTab = "swap" | "transfer";

// ── Tab wrapper (add at top of returned JSX, before existing header) ──
...
interface NftKey {
  ...
  vaultRef?:     string;
  lockerRef?:    string;
  isTicket?:     boolean;          // true = event ticket NFT
  transferLock?: number;           // days remaining on transfer lock (0 = unlocked)
  kycRequired?:  "soft" | "standard" | "hard" | null; // KYC level required to receive
  eventName?:    string;           // event name if ticket
}
...
const MOCK_MY_NFTS: NftKey[] = [
  ...
  { mint: "7xK2...9mN", name: "NexusKey #4821",
    vaultRef: "VLT-4729...881", lockerRef: "LCK-0000...001",
    isTicket: false, transferLock: 0, kycRequired: null },
  { mint: "3bR8...1pQ", name: "NexusKey #0293",
    vaultRef: "VLT-8821...443", lockerRef: "LCK-0000...001",
    isTicket: false, transferLock: 0, kycRequired: null },
  // Ticket NFT — transfer locked (18 days remaining)
  { mint: "9mK2...7pR", name: "🎵 #021*025-15",
    isTicket: true, transferLock: 18, kycRequired: "soft",
    eventName: "The Midnight — MSG, June 14 2027" },
  // Ticket NFT — lock expired, KYC required to receive
  { mint: "4bX8...2qL", name: "🎵 #VIP-014",
    isTicket: true, transferLock: 0, kycRequired: "hard",
    eventName: "The Midnight — MSG, June 14 2027" },
];
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-8">
                ...
                {showNftPicker && (
                  <div className="rounded-xl border border-white/10 bg-black/80 overflow-hidden">
                    {MOCK_MY_NFTS.map(nft => {
                      const isLocked = (nft.transferLock ?? 0) > 0;
                      return (
                        <button
                          key={nft.mint}
                          onClick={() => { if (!isLocked) { setMyNft(nft); setShowNftPicker(false); } }}
                          disabled={isLocked}
                          className={`w-full flex items-center gap-3 p-3 border-b border-white/5 last:border-0 transition-colors text-left
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {nft.isTicket && (
                                <span className="text-xs px-1.5 py-0.5 rounded-full bg-monad-purple/20 text-monad-purple border border-monad-purple/20">Ticket</span>
                              )}
                              {isLocked && (
                                <span className="text-xs px-1.5 py-0.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 flex items-center gap-1">
                                  <Lock className="h-2.5 w-2.5" />{nft.transferLock}d locked
                                </span>
                              )}
                              {!isLocked && nft.kycRequired && (
                                <span className="text-xs px-1.5 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
                                  {nft.kycRequired} KYC receiver
                                </span>
                              )}
                            </div>
                            {nft.eventName
                              ? <p className="text-xs text-gray-600 truncate mt-0.5">{nft.eventName}</p>
                              : <p className="text-xs text-gray-600 font-mono">{nft.vaultRef}</p>
                            }
                            {isLocked && <p className="text-xs text-red-400/70 mt-0.5">Cannot swap — {nft.transferLock} days remaining on transfer lock</p>}
                          </div>
                        </button>
                      );
                    })}
        ...
        {step === "review" && (
          <div className="space-y-4">
            ...
            {/* Consent statement */}
            {/* Ticket NFT KYC warning */}
            {myNft?.isTicket && myNft?.kycRequired && (
              <div className="p-4 rounded-xl border border-monad-purple/20 bg-monad-purple/5 flex gap-2">
                <AlertTriangle className="h-4 w-4 text-monad-purple shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <p className="text-xs font-bold text-monad-purple">Ticket NFT — KYC required on receiving wallet</p>
                  <p className="text-xs text-gray-400">
                    This is an event ticket NFT. The counterparty's wallet must hold a valid soul-bound Identity Token at{" "}
                    <span className="font-bold text-white">{myNft.kycRequired}</span> verification level or higher.
                    If they do not, the transfer will be rejected by the contract.
                  </p>
                  {myNft.eventName && (
                    <p className="text-xs text-gray-500 font-mono">{myNft.eventName}</p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="flex gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <ul className="text-xs text-yellow-500/70 space-y-1">
                    ...
                    {myNft?.isTicket && (
                      <li>• This ticket NFT carries event-specific transfer rules enforced at the contract level</li>
                    )}
                  </ul>
                </div>
              </div>
