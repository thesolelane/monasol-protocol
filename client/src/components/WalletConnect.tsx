      ...
      >
        <div
          className={`rounded-full p-2 ${
            isConnected ? (isEvm ? "bg-monad-purple/20" : "bg-solana-green/20") : "bg-white/5"
          }`}
        >
          {isConnected ? <CheckCircle2 className="h-5 w-5" /> : <Wallet className="h-5 w-5" />}
        </div>
        ...
        {isConnected && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2">
            <ShieldCheck
              className={`h-5 w-5 ${isEvm ? "text-monad-purple" : "text-solana-green"}`}
            />
          </div>
        )}
      </Button>
