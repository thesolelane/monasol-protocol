...
1. **Compartmentalized Risk (The "Apartment Building" Model):** Instead of a single monolithic smart contract holding all user funds, NexusBridge deploys isolated smart contracts called **"Lockers"**.
   ...
   * **Sub-Vault Gating (The "Rooms" Model):** Inside a single Apartment (Vault), the owner can create distinct **"Rooms"** (subdomains/sub-vaults). This allows the vault owner to partition their assets internally and grant highly granular access. For example, the owner holds the master key to the Apartment, but they can issue a temporary, restricted key that only opens "Room A" (which holds 100 USDC for daily expenses), while "Room B" (holding 50,000 USDC) remains completely locked and inaccessible to the restricted key holder.
