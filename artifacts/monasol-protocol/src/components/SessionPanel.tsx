...
export function SessionPanel({ vaultId, nftName }: SessionPanelProps) {
  ...
  const timeLeft = session ? session.openedAt + session.durationMs - now : 0;
  const pct = session ? Math.max(0, timeLeft / session.durationMs) : 0;
  ...
  async function handleOpen() {
    ...
    setSession({
      id: shortId(),
      authorizedAddress: authAddress.trim() || "Any holder",
      openedAt: Date.now(),
      durationMs: DURATION_OPTIONS[durationIdx].ms,
      label: sessionLabel.trim() || "General session",
    });
    setLoading(null);
    setSuccess("opened");
    ...
    <div className="glass-panel rounded-2xl p-6 sm:p-8 relative overflow-hidden mt-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-bold ${
          ...
          {session && !expired ? "OPEN" : expired ? "EXPIRED" : "CLOSED"}
        </div>
      </div>
          ...
          >
            ...
            <div>
              ...
              <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                <motion.div
                  className="h-full rounded-full bg-monad-purple"
                  style={{ width: `${pct * 100}%` }}
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 divide-y divide-white/5 text-sm">
              ...
              <div className="px-4 py-2.5 flex justify-between">
                ...
                <span className="text-white">{DURATION_OPTIONS.find(d => d.ms === session.durationMs)?.label ?? "Custom"}</span>
              </div>
              <div className="px-4 py-2.5 flex justify-between">
            ...
            <AnimatePresence>
              {success === "opened" && (
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <CheckCircle className="h-3.5 w-3.5" /> Session opened on-chain
                </motion.div>
              )}
            </AnimatePresence>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <button
                  type="button"
                  data-testid="button-session-duration"
                    ...
                    >
                      {DURATION_OPTIONS.map((opt, i) => (
                        <button
                          key={i}
                          type="button"
                          data-testid={`option-duration-${i}`}
                          onClick={() => { setDurationIdx(i); setDurationOpen(false); }}
                          className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 hover:bg-monad-purple/20 transition-colors ${
                            i === durationIdx ? "text-monad-purple font-semibold" : "text-gray-300"
                          }`}
                        >
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  data-testid="input-session-auth-address"
            ...
            <AnimatePresence>
              {success === "closed" && (
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <CheckCircle className="h-3.5 w-3.5 text-solana-green" /> Session closed on-chain
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <p className="text-[11px] text-gray-600 text-center leading-relaxed">
              ...
              Sessions gate interactions with your vault for the chosen duration.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
